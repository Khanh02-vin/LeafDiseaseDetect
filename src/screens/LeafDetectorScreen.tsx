...
export const LeafDetectorScreen: React.FC = () => {
  ...
  const analyzeImage = useCallback(async () => {
    ...
    try {
      ...
      // ── GPS capture (respects locationTracking setting) ──
      let location: { latitude: number; longitude: number; address?: string } | undefined;
      if (settings.locationTracking) {
        Logger.info(LogCategory.CLASSIFICATION, 'Capturing GPS location...');
        const loc = await locationService.getCurrentLocation();
        if (loc) {
          location = loc;
          Logger.success(LogCategory.CLASSIFICATION, `Location captured: ${loc.latitude.toFixed(4)}, ${loc.longitude.toFixed(4)}`);
        }
      }

      Logger.info(LogCategory.CLASSIFICATION, `Analyzing ${imageSource} image`);
      const result = await leafClassifier.classifyLeaf(capturedImage, location, {
        source: imageSource,
      });
      ...
      // ── Send notification if result is a disease ──
      if (result.primaryResult.isLeaf) {
        await notificationService.sendDiseaseAlert(
          settings.notifications,
          result.primaryResult.label,
          result.primaryResult.confidence,
        );
      }
    } catch (error) {
  ...
  }, [capturedImage, imageSource, leafClassifier, settings, setCurrentScan, addToHistory, setLoading, isModelReady]);
